...
function postLayout(callback, oc){
  Ti.API.log('$.pagingcontrol.size.width ' + $.pagingcontrol.size.width +' oc ' + oc);
    Ti.API.log('test asPostLayout '+ asPostLayout);
    if(!asPostLayout){
      callback();
      asPostLayout = true;
    }
    ...
    /*if (!oc && $.pagingcontrol.size.width){
      Ti.API.log('je passe ici');
        callback();
    }else{
      Ti.API.log('je passe là');
...
function updateOffset(index){
  ...
  if (width < tabsWidth){
    ...
    newOffset = OS_IOS ? (offsetDp < 0 ? 0 : offsetDp) : (offsetDp < 0 ? 0 : offsetDp) ;
    if (OS_ANDROID) {
