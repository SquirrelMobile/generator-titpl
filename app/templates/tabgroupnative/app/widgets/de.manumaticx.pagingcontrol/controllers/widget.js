...
function postLayout(callback, oc) {
  // Alloy.Globals.log.info('$.pagingcontrol.size.width ' + $.pagingcontrol.size.width +' oc ' + oc);
  // Alloy.Globals.log.info('test asPostLayout '+ asPostLayout);
  if (!asPostLayout) {
    callback();
    asPostLayout = true;
  }
  ...
  /*if (!oc && $.pagingcontrol.size.width){
      Alloy.Globals.log.info('je passe ici');
        callback();
    }else{
      Alloy.Globals.log.info('je passe là');
        ...
        $.pagingcontrol.addEventListener('postlayout', function onPostLayout(evt){
          Alloy.Globals.log.info('postlayout');
            // callback
            callback();
 ...
 */
function init() {
  Alloy.Globals.log.info("--- args " + JSON.stringify(args));
  if (args.tabs) {
    // create tabs
