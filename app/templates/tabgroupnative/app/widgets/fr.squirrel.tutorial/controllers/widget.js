...
var defaultIndicatorSelect = {
	backgroundColor: "black",
};
var defaultIndicatorUnselect = {
...
$.setPages = function(obj) {
	...
	if (obj.length > 0) {
		...
		if (args.indicatorSelect && args.indicatorUnselect) {
			...
			setIndicators({
				indicatorSelect: defaultIndicatorSelect,
				indicatorUnselect: defaultIndicatorUnselect,
			});
		}
...
function setIndicators(obj) {
	...
	_.each(tabPage, function(page) {
		...
		if (i === 0) {
			...
			var img = $.UI.create(
				"View",
				_.extend(objDefault, _.omit(obj.indicatorSelect, ["id", "left"])),
			);
			...
			var img2 = $.UI.create(
				"View",
				_.extend(objDefault, _.omit(obj.indicatorUnselect, ["id", "left"])),
			);
			// on ajoute les indicateurs de page à la vue parent
			$.indicators.add(img2);
		}
...
function changePage(e) {
	...
	if (valeur > 0 && valeur < 0.5) {
		// << Entre le debut du scroll et la moitié du scroll
