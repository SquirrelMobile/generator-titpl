...
(function() {
  ...
  Alloy.Globals.Cache = (function() {
    var init_cache, expire_cache, current_timestamp, get, put, del;

    // Cache initialization
    init_cache = function(cache_expiration_interval) {
      var db = Titanium.Database.open("databasetest");
      db.execute(
        "CREATE TABLE IF NOT EXISTS cache (key TEXT UNIQUE, value TEXT, expiration INTEGER)"
      );
      db.close();
      ...
      if (!CONFIG || (CONFIG && !CONFIG.EXPIRE_ON_GET)) {
        ...
        // Ti.API.info(
        //   "[CACHE] Will expire objects each " +
    ...
    expire_cache = function() {
      ...
      var count = 0;
      var rs = db.execute(
        "SELECT COUNT(*) FROM cache WHERE expiration <= ?",
        timestamp
      );
      while (rs.isValidRow()) {
        count = rs.field(0);
